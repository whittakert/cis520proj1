...
process_execute (const char *file_name) 
{
  ...
  /* Copy file_name again for parsing. */
  ...
  palloc_free_page (copy2); //was only used for parsing
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Copy file_name for parsing. */
  char *nameCopy = palloc_get_page (0);
  if (nameCopy == NULL)
    return TID_ERROR;
  strlcpy (nameCopy, file_name, PGSIZE);

  char *save_ptr;
  char *fileName = strtok_r(nameCopy, " ", &save_ptr); //Separate the filename
  ...
  palloc_free_page (nameCopy); //was only used for parsing
...
setup_stack (void **esp, char *file_name) 
{
  ...
  /* Now push the correct values and addresses onto the stack from
     the file_name. */
